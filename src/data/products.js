...
const storeSearchUrl = (storeName, productName) => {
  ...
  const map = {
    ...
    'Hasaki': `https://hasaki.vn/catalogsearch/result?q=${q}`,
    'FunnyLand': `https://www.funnyland.vn/search?q=${q}`,
    'ToyZone': `https://www.toyzone.vn/search?q=${q}`
  };
...
const offlineByCategory = (category, base) => {
  ...
  if (category === 'Mỹ phẩm') {
    return [
      offer('Hasaki', 'offline', Math.round(base * 1.02), 0, 18000, 0, 'Đã có tài khoản'),
  ...
  if (category === 'Đồ chơi') {
    return [
      ...
      offer('FunnyLand', 'offline', Math.round(base * 1.05), 0, 9000, 0, 'Đã có tài khoản'),
      offer('ToyZone', 'offline', Math.round(base * 1.04), 0, 10000, 0, 'Đã có tài khoản')
    ];
  }
...
// points: số điểm dữ liệu muốn tạo (mặc định 7 — tương thích bản cũ). Cawi Tín Hiệu Mua
// (v63) gọi với points lớn hơn để mô phỏng lịch sử dài tới 180 ngày cho gói CartWise Plus.
export const getPriceHistory = (product, storeName, points = 7) => {
  const availableStores = (product?.stores || []).filter((store) => store.available !== false && store.storePrice != null);
  const selectedStore = availableStores.find((store) => store.storeName === storeName) || getBestFinalStore(product) || getBestStore(product);
  const current = selectedStore ? Number(selectedStore.storePrice || 0) : Number(product?.basePrice || 0);
  if (!current) return [];

  const seed = hashNumber(`${product?.id || product?.name || ''}-${selectedStore?.storeName || storeName || 'best'}`);
  const count = Math.max(2, Math.round(points));
  const basePattern = Array.from({ length: count }, (_, index) => {
    // Dao động hình sin nhẹ quanh 1.0 để tạo cảm giác xu hướng tăng/giảm theo thời gian,
    // vẫn hội tụ về giá hiện tại (current) ở điểm cuối cùng.
    const progress = index / (count - 1);
    return 1 + 0.06 * Math.sin(progress * Math.PI * 2 + (seed % 7)) * (1 - progress) + 0.015 * ((seed % 5) - 2) * (1 - progress);
  });

  return basePattern.map((factor, index) => {
    const variation = (((seed + index * 17) % 7) - 3) / 200;
    const value = index === count - 1
      ? current
      : Math.max(0, Math.round(current * (factor + variation)));
    const daysAgo = count - 1 - index;
    return {
      storeName: selectedStore?.storeName || storeName || 'CartWise',
      label: daysAgo === 0 ? 'Hôm nay' : `${daysAgo} ngày trước`,
      daysAgo,
      value,
...
export const getPriceInsight = (product, storeName) => {
  const history = getPriceHistory(product, storeName, 7);
  if (!history.length) {
    return {
...
// v63 — "Cawi Tín Hiệu Mua": khuyến nghị Mua ngay / Nên chờ dựa trên lịch sử giá tích luỹ
// tối đa 180 ngày (chỉ mở khoá ở CartWise Plus — Mục 4.2 báo cáo cải tiến Vòng 4).
// Đây vẫn là công thức minh bạch (không gọi AI thật): so giá hiện tại với mức thấp nhất/
// trung bình/cao nhất trong lịch sử quan sát được, độ tin cậy tăng theo độ dài lịch sử.
export const getBuySignal = (product, storeName, historyDays = 180) => {
  const points = Math.max(14, Math.min(60, Math.round(historyDays / 3)));
  const history = getPriceHistory(product, storeName, points);
  if (!history.length) {
    return { recommendation: 'neutral', confidence: 0, headline: 'Chưa đủ dữ liệu', detail: 'Cần thêm lịch sử giá để đưa ra khuyến nghị.', history };
  }
  ...
  const percentile = Math.round(((current - min) / range) * 100); // 0 = rẻ nhất quan sát được, 100 = đắt nhất

  // Độ tin cậy tăng theo số điểm dữ liệu (lịch sử dài hơn) và độ biến động quan sát được.
  const volatility = range / Math.max(1, average);
  const confidence = Math.min(97, Math.round(55 + (points / 60) * 30 + Math.min(12, volatility * 100)));

  if (percentile <= 20) {
    return {
      recommendation: 'buy',
      confidence,
      headline: `Giá đang thấp nhất trong ${historyDays} ngày qua`,
      detail: `Mức giá hiện tại gần với đáy quan sát được trong ${historyDays} ngày (thấp hơn trung bình ${Math.max(0, Math.round((1 - current / average) * 100))}%). Đây là thời điểm nên mua nếu bạn đang cần sản phẩm này.`,
  ...
  if (percentile >= 75) {
    return {
      recommendation: 'wait',
      confidence,
      headline: `Giá đang cao hơn phần lớn ${historyDays} ngày qua`,
      detail: `Giá hiện tại cao hơn khoảng ${Math.max(0, Math.round((current / average - 1) * 100))}% so với trung bình ${historyDays} ngày. Nên chờ đợt giảm giá tiếp theo hoặc theo dõi thêm nếu không quá gấp.`,
      current, min, max, average, percentile, history
    };
  }
  ...
  return {
    ...
    confidence,
    headline: 'Giá đang ở mức trung bình',
    detail: `Giá hiện tại không chênh nhiều so với mức trung bình ${historyDays} ngày qua. Có thể mua nếu phù hợp nhu cầu, hoặc chờ thêm nếu muốn giá tốt hơn.`,
    current, min, max, average, percentile, history
  };
};
